import 'package:i_health/core/constants/assets.dart';
import 'package:i_health/feature/home/data/model/home_data_model.dart';

class HomeData {
  static List<HomeDataModel> homeData = [
    HomeDataModel(
      title: 'How Can Artificial Intelligence Help You Diagnose Diseases?',
      description: '''
How Does Artificial Intelligence Work in Diagnosis?
In recent years, artificial intelligence (AI) has revolutionized the field of medicine. Its role is no longer limited to surgical robots or radiology analysis but has expanded to include early disease diagnosis. By analyzing vast amounts of medical data, AI can help patients quickly identify their symptoms and guide them to the appropriate medical specialty. In this article, we will explore how AI works in medical diagnosis, the key tools used, and how you can benefit from them.

🩺 How Does Artificial Intelligence Work in Diagnosis?
AI relies on big data analysis to diagnose diseases and operates using several advanced technologies, including:

 Symptom Analysis and Disease Prediction
📌 When symptoms are entered into an AI-based system, they are compared with thousands of similar cases to derive a potential diagnosis.
📌 Some systems use deep learning to analyze medical images (such as X-rays) and identify diseases with high accuracy.

2️⃣ Predicting Chronic Diseases Before They Appear
📌 AI can predict the likelihood of diseases such as diabetes, heart disease, and cancer based on genetic factors and lifestyle patterns.
📌 It helps doctors make early preventive decisions to reduce the risk of disease progression.

🛠️ Examples of AI Tools in Medicine
1️⃣ Medical Chatbots (Like Your Project! 👀)
🤖 How Does It Work?
✅ The user enters their symptoms into the chatbot.
✅ The system analyzes the symptoms and compares them with a vast medical database.
✅ It displays the potential disease name and directs the user to the appropriate medical specialty.

📌 Real-Life Example:
ChatGPT in the medical field is used to analyze patient inquiries and provide initial guidance.''',
      imageUrl: Assets.assetsImagesFirst,
    ),
    HomeDataModel(
      title:
          'How to Improve Your Lifestyle to Avoid the 5 Most Common Diseases?',
      description: '''
In our modern era, an unhealthy lifestyle has become a major cause of many chronic diseases. The good news is that simple adjustments to your diet and daily routine can significantly reduce the risk of these diseases. In this article, we will explore 5 common diseases and how to prevent them through healthy lifestyle changes.

1️⃣ Diabetes 🩸
🔹 Why Does It Happen?
Type 2 diabetes is often caused by an unhealthy lifestyle, such as consuming sugar-rich foods and lack of physical activity.

💡 How to Avoid It?
✅ Eat fewer refined carbohydrates (like white bread and sweets).
✅ Increase your intake of fiber-rich foods like vegetables and oats.
✅ Exercise for 30 minutes daily (walking, running, or swimming).
✅ Drink water instead of sodas and canned juices.


2️⃣ High Blood Pressure (Hypertension) 🫀
🔹 Why Does It Happen?
Consuming large amounts of salt, lack of movement, and chronic stress can lead to high blood pressure, increasing the risk of heart attacks.

💡 How to Avoid It?
✅ Reduce salt and sodium intake (avoid fast food and canned foods).
✅ Eat potassium-rich fruits and vegetables like bananas and spinach.
✅ Avoid excessive caffeine-rich drinks like coffee.
✅ Practice relaxation exercises like yoga and deep breathing to reduce stress.


3️⃣ Stomach Ulcer 🤕
🔹 Why Does It Happen?
One of the main causes of stomach ulcers is excessive use of painkillers or infection with H. pylori bacteria, in addition to stress and smoking.

💡 How to Avoid It?
✅ Avoid spicy and fried foods that irritate the stomach.
✅ Do not overuse painkillers like ibuprofen without consulting a doctor.
✅ Eat yogurt and probiotic-rich foods to support digestive health.
✅ Reduce stress and anxiety as they increase the risk of ulcers.


4️⃣ Anemia 🩸
🔹 Why Does It Happen?
Anemia results from a deficiency of iron or vitamins in the body, leading to fatigue and dizziness.

💡 How to Avoid It?
✅ Eat iron-rich foods like liver, spinach, lentils, and legumes.
✅ Drink orange juice after meals to enhance iron absorption.
✅ Avoid tea and coffee immediately after eating as they reduce iron absorption.
✅ Conduct regular tests for iron and hemoglobin levels.


5️⃣ Arthritis 🦴
🔹 Why Does It Happen?
Sitting for long periods, lack of physical activity, and deficiency of certain nutrients can lead to arthritis with age.

💡 How to Avoid It?
✅ Exercise regularly, such as walking and stretching.
✅ Eat foods rich in omega-3 fatty acids like fish and nuts.
✅ Avoid excess weight as it increases pressure on the joints.
✅ Drink enough water to maintain joint flexibility.

💡 Conclusion
"Prevention is better than cure," and adopting a healthy lifestyle can protect you from many chronic diseases. Don’t wait for symptoms to appear; start today by applying these healthy habits for a better and more active life!''',
      imageUrl: Assets
          .assetsImagesSecond, // Replace with the correct image asset path
    ),
    HomeDataModel(
      title: '🛡️ Best Foods to Boost Immunity and Prevent Diseases',
      description: '''
🥦 Foods That Boost Immunity
Some foods contain vitamins and minerals that support the immune system, helping your body fight viruses and infections. Here are some of the most important ones:

1️⃣ Vitamin C-Rich Foods 🍊
Why? Vitamin C enhances the production of white blood cells, which fight infections.

✅ Oranges 🍊
✅ Strawberries 🍓
✅ Kiwi 🥝
✅ Red Bell Peppers 🌶️

2️⃣ Zinc-Rich Foods 🥜
Why? Zinc helps speed up wound healing and strengthens the immune system.

✅ Nuts (almonds, walnuts)
✅ Seeds (flaxseeds, chia seeds)
✅ Lean red meat
✅ Oysters and fish 🦪

3️⃣ Probiotic-Rich Foods 🥛
Why? Probiotics improve gut health, where more than 70% of the immune system resides!

✅ Natural yogurt
✅ Natural pickles
✅ Kefir (a yogurt-like drink)

4️⃣ Antioxidant-Rich Foods 🫐
Why? Antioxidants fight inflammation and protect the body's cells from damage.

✅ Blueberries 🫐
✅ Green tea 🍵
✅ Carrots 🥕
✅ Tomatoes 🍅

🚫 Foods That Weaken Immunity and Should Be Avoided
Unfortunately, some foods can weaken the immune system and make you more susceptible to diseases.

1️⃣ Refined Sugars 🍭
❌ Sweets and sodas reduce the ability of immune cells to fight viruses for up to 5 hours after consumption!

2️⃣ Processed Foods �
❌ Processed meats like sausages and deli meats contain preservatives that can affect the immune system.

3️⃣ Saturated and Trans Fats 🍔
❌ Fried foods and fast food cause chronic inflammation in the body, weakening immunity.

4️⃣ Alcohol and Excessive Caffeine ☕
❌ Excessive coffee and alcohol can cause dehydration and weaken immune responses.

💪 Healthy Habits to Maintain Strong Immunity
In addition to good nutrition, here are some habits to follow to boost your immunity:

✅ Get enough sleep (7-9 hours daily) 😴
✅ Exercise regularly �
✅ Drink plenty of water 💧
✅ Reduce stress through meditation or breathing exercises 🧘‍♀️
✅ Wash your hands regularly to prevent infections 🧼

💡 Conclusion
To strengthen your body against diseases, eat foods rich in vitamins and minerals like fruits and vegetables, and avoid sugars and processed foods. Don’t forget to adopt healthy habits like good sleep and exercise to maintain strong immunity.''',
      imageUrl:
          Assets.assetsImagesThird, // Replace with the correct image asset path
    ),
  ];

  static List<HomeDataModel> homeDataArabic = [
    HomeDataModel(
      title: 'كيف يمكن للذكاء الاصطناعي مساعدتك في تشخيص الأمراض؟',
      description: '''
كيف يعمل الذكاء الاصطناعي في التشخيص؟
في السنوات الأخيرة، أحدث الذكاء الاصطناعي (AI) ثورة في مجال الطب. لم يعد دوره يقتصر على الروبوتات الجراحية أو تحليل الأشعة فقط، بل امتد ليشمل التشخيص المبكر للأمراض. من خلال تحليل كميات هائلة من البيانات الطبية، يمكن للذكاء الاصطناعي مساعدة المرضى في تحديد أعراضهم بسرعة وتوجيههم إلى التخصص الطبي المناسب. في هذه المقالة، سنستكشف كيف يعمل الذكاء الاصطناعي في التشخيص الطبي، والأدوات الرئيسية المستخدمة، وكيف يمكنك الاستفادة منها.

🩺 كيف يعمل الذكاء الاصطناعي في التشخيص؟
يعتمد الذكاء الاصطناعي على تحليل البيانات الضخمة لتشخيص الأمراض ويعمل باستخدام عدة تقنيات متقدمة، بما في ذلك:

1️⃣ تحليل الأعراض وتوقع الأمراض
📌 عند إدخال الأعراض في نظام يعتمد على الذكاء الاصطناعي، تتم مقارنتها مع آلاف الحالات المشابهة للتوصل إلى تشخيص محتمل.
📌 تستخدم بعض الأنظمة التعلم العميق لتحليل الصور الطبية (مثل الأشعة السينية) وتحديد الأمراض بدقة عالية.

2️⃣ توقع الأمراض المزمنة قبل ظهورها
📌 يمكن للذكاء الاصطناعي توقع احتمالية الإصابة بأمراض مثل السكري وأمراض القلب والسرطان بناءً على العوامل الوراثية وأنماط الحياة.
📌 يساعد الأطباء في اتخاذ قرارات وقائية مبكرة لتقليل خطر تطور المرض.

🛠️ أمثلة على أدوات الذكاء الاصطناعي في الطب
1️⃣ روبوتات المحادثة الطبية (مثل مشروعك! 👀)
🤖 كيف تعمل؟
✅ يدخل المستخدم أعراضه في روبوت المحادثة.
✅ يحلل النظام الأعراض ويقارنها مع قاعدة بيانات طبية ضخمة.
✅ يعرض اسم المرض المحتمل ويوجه المستخدم إلى التخصص الطبي المناسب.

📌 مثال من الواقع:
يستخدم ChatGPT في المجال الطبي لتحليل استفسارات المرضى وتقديم إرشادات أولية.''',
      imageUrl: Assets.assetsImagesSecond,
    ),
    HomeDataModel(
      title: 'كيف تحسن نمط حياتك لتجنب أكثر 5 أمراض شيوعًا؟',
      description: '''
في عصرنا الحديث، أصبح نمط الحياة غير الصحي سببًا رئيسيًا للعديد من الأمراض المزمنة. الخبر السار هو أن تعديلات بسيطة على نظامك الغذائي وروتينك اليومي يمكن أن تقلل بشكل كبير من خطر هذه الأمراض. في هذه المقالة، سنستكشف 5 أمراض شائعة وكيفية الوقاية منها من خلال تغييرات صحية في نمط الحياة.

1️⃣ السكري 🩸
🔹 لماذا يحدث؟
غالبًا ما يحدث النوع الثاني من السكري بسبب نمط حياة غير صحي، مثل تناول الأطعمة الغنية بالسكر وقلة النشاط البدني.

💡 كيف تتجنبه؟
✅ تناول كميات أقل من الكربوهيدرات المكررة (مثل الخبز الأبيض والحلويات).
✅ زد من تناول الأطعمة الغنية بالألياف مثل الخضروات والشوفان.
✅ مارس التمارين لمدة 30 دقيقة يوميًا (المشي، الجري، أو السباحة).
✅ اشرب الماء بدلاً من المشروبات الغازية والعصائر المعلبة.

2️⃣ ارتفاع ضغط الدم 🫀
🔹 لماذا يحدث؟
تناول كميات كبيرة من الملح، قلة الحركة، والإجهاد المزمن يمكن أن يؤدي إلى ارتفاع ضغط الدم، مما يزيد من خطر النوبات القلبية.

💡 كيف تتجنبه؟
✅ قلل من تناول الملح والصوديوم (تجنب الوجبات السريعة والأطعمة المعلبة).
✅ تناول الفواكه والخضروات الغنية بالبوتاسيوم مثل الموز والسبانخ.
✅ تجنب المشروبات الغنية بالكافيين مثل القهوة بكميات كبيرة.
✅ مارس تمارين الاسترخاء مثل اليوجا والتنفس العميق لتقليل التوتر.

3️⃣ قرحة المعدة 🤕
🔹 لماذا يحدث؟
أحد الأسباب الرئيسية لقرحة المعدة هو الاستخدام المفرط للمسكنات أو الإصابة ببكتيريا H. pylori، بالإضافة إلى التوتر والتدخين.

💡 كيف تتجنبه؟
✅ تجنب الأطعمة الحارة والمقلية التي تهيج المعدة.
✅ لا تفرط في استخدام المسكنات مثل الإيبوبروفين دون استشارة الطبيب.
✅ تناول الزبادي والأطعمة الغنية بالبروبيوتيك لدعم صحة الجهاز الهضمي.
✅ قلل من التوتر والقلق لأنها تزيد من خطر القرحة.

4️⃣ فقر الدم 🩸
🔹 لماذا يحدث؟
يحدث فقر الدم نتيجة نقص الحديد أو الفيتامينات في الجسم، مما يؤدي إلى التعب والدوخة.

💡 كيف تتجنبه؟
✅ تناول الأطعمة الغنية بالحديد مثل الكبد والسبانخ والعدس والبقوليات.
✅ اشرب عصير البرتقال بعد الوجبات لتعزيز امتصاص الحديد.
✅ تجنب الشاي والقهوة مباشرة بعد الأكل لأنها تقلل من امتصاص الحديد.
✅ أجرِ فحوصات منتظمة لمستويات الحديد والهيموجلوبين.

5️⃣ التهاب المفاصل 🦴
🔹 لماذا يحدث؟
الجلوس لفترات طويلة، قلة النشاط البدني، ونقص بعض العناصر الغذائية يمكن أن يؤدي إلى التهاب المفاصل مع التقدم في العمر.

💡 كيف تتجنبه؟
✅ مارس التمارين بانتظام، مثل المشي وتمارين التمدد.
✅ تناول الأطعمة الغنية بأحماض أوميغا-3 الدهنية مثل الأسماك والمكسرات.
✅ تجنب الوزن الزائد لأنه يزيد الضغط على المفاصل.
✅ اشرب كمية كافية من الماء للحفاظ على مرونة المفاصل.

💡 خاتمة
"الوقاية خير من العلاج"، واعتماد نمط حياة صحي يمكن أن يحميك من العديد من الأمراض المزمنة. لا تنتظر ظهور الأعراض؛ ابدأ اليوم بتطبيق هذه العادات الصحية لحياة أفضل وأكثر نشاطًا!''',
      imageUrl: Assets.assetsImagesThird,
    ),
    HomeDataModel(
      title: '🛡️ أفضل الأطعمة لتقوية المناعة والوقاية من الأمراض',
      description: '''
🥦 أطعمة تعزز المناعة
بعض الأطعمة تحتوي على فيتامينات ومعادن تدعم جهاز المناعة، وتساعد جسمك في محاربة الفيروسات والعدوى. إليك أهم هذه الأطعمة:

1️⃣ الأطعمة الغنية بفيتامين C 🍊  
📌 لماذا؟ لأن فيتامين C يعزز إنتاج خلايا الدم البيضاء التي تحارب العدوى.

✅ البرتقال  
✅ الفراولة  
✅ الكيوي  
✅ الفلفل الأحمر

2️⃣ الأطعمة الغنية بالزنك 🥜  
📌 لماذا؟ الزنك يسرّع التئام الجروح ويقوي جهاز المناعة.

✅ المكسرات (اللوز، الجوز)  
✅ البذور (الكتان، الشيا)  
✅ اللحوم الحمراء الخالية من الدهون  
✅ المحار والأسماك 🦪

3️⃣ الأطعمة الغنية بالبروبيوتيك 🥛  
📌 لماذا؟ البروبيوتيك تحسن صحة الأمعاء، حيث يوجد أكثر من 70٪ من جهاز المناعة!

✅ الزبادي الطبيعي  
✅ المخللات الطبيعية  
✅ الكفير (مشروب يشبه الزبادي)

4️⃣ الأطعمة الغنية بمضادات الأكسدة 🫐  
📌 لماذا؟ مضادات الأكسدة تحارب الالتهابات وتحمي خلايا الجسم من التلف.

✅ التوت الأزرق  
✅ الشاي الأخضر  
✅ الجزر  
✅ الطماطم

🚫 أطعمة تضعف المناعة ويُفضل تجنبها  
بعض الأطعمة قد تُضعف جهاز المناعة وتزيد من احتمالية الإصابة بالأمراض.

1️⃣ السكريات المكررة 🍭  
❌ الحلويات والمشروبات الغازية تقلل من قدرة خلايا المناعة على محاربة الفيروسات لمدة تصل إلى 5 ساعات بعد تناولها!

2️⃣ الأطعمة المعالجة  
❌ اللحوم المصنعة مثل السجق واللانشون تحتوي على مواد حافظة تؤثر سلبًا على المناعة.

3️⃣ الدهون المشبعة والمتحولة 🍔  
❌ الأطعمة المقلية والوجبات السريعة تسبب التهابات مزمنة في الجسم وتضعف المناعة.

4️⃣ الكحول والكافيين الزائد ☕  
❌ الإفراط في شرب القهوة والكحول يسبب الجفاف ويضعف الاستجابة المناعية.

💪 عادات صحية للحفاظ على مناعة قوية  
بجانب التغذية السليمة، إليك بعض العادات التي تقوي جهازك المناعي:

✅ النوم الكافي (7-9 ساعات يوميًا) 😴  
✅ ممارسة الرياضة بانتظام  
✅ شرب كميات كافية من الماء 💧  
✅ تقليل التوتر من خلال التأمل أو تمارين التنفس 🧘‍♀️  
✅ غسل اليدين بانتظام للوقاية من العدوى 🧼

💡 خلاصة  
لتقوية جسمك ضد الأمراض، ركز على تناول أطعمة غنية بالفيتامينات والمعادن مثل الفواكه والخضروات، وابتعد عن السكريات والأطعمة المعالجة. ولا تنسَ اعتماد عادات صحية مثل النوم الجيد وممارسة الرياضة للحفاظ على مناعة قوية.
''',
      imageUrl: Assets.assetsImagesThird,
    ),
  ];
}
